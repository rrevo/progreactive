...
trait WikipediaApi {
  ...
  implicit class StringObservableOps(obs: Observable[String]) {
     ...
     */
    def sanitized: Observable[String] = obs map (str => str.replaceAll(" ", "_"))
  ...
  implicit class ObservableOps[T](obs: Observable[T]) {
    ...
    /**
     * Returns a new observable that contains all the events from obs that happened before totalSec seconds elapse.
     * If the obs combinator is completed or failed before the timeout, the resulting observable should also be
     * completed or failed, respectively.
     * After `totalSec` seconds, if `obs` is not yet completed, the result observable becomes completed.
     *
     * Note: uses the existing combinators on observables.
     */
    def timedOut(totalSec: Long): Observable[T] = {
      val timer = Observable.interval(totalSec second).take(1)
      obs.takeUntil(timer)
    }
    ...
    def concatRecovered[S](requestMethod: T => Observable[S]): Observable[Try[S]] = {
      val ss = obs.map(t => requestMethod(t).recovered)
      val ssc = ss.concat
      ssc
    }
