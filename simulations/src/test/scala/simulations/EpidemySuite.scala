...
class EpidemySuite extends FunSuite {
  ...
  test("prevalence rate") {
    ...
    assert(es.SimConfig.population == es.persons.length)
  ...
  test("transmissibility rate") {
    ...
    for (i <- 0 to 100) {
      ...
      // Find the first healthy person
      val healthyPerson = (es.persons find { p => !p.infected }).get
      
      // Make all others infected
    ...
    assert(infectedTimes > 0,
      "A person should get infected according to the transmissibility rate when he moves into a room with an infectious person")
  }
